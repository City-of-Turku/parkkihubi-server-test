...
from constants import PARKKIOPAS_HOST, TEST_EVENT_PARKING
...
def test_get_parking_area_statistics_by_id(id):
    response = requests.get(
        f"{PARKKIOPAS_HOST}/public/v1/parking_area_statistics/{id}/"
    )
    assert response.status_code == 200, response.text
    json_data = response.json()
...
def test_get_list_of_event_areas_statistics():
    response = requests.get(f"{PARKKIOPAS_HOST}/public/v1/event_area_statistics/")
    assert response.status_code == 200, response.text
    json_data = response.json()
    assert "count" in json_data
    assert "next" in json_data
...
def test_get_list_of_event_areas_total_statistics():
    response = requests.get(f"{PARKKIOPAS_HOST}/public/v1/event_area_total_statistics/")
    assert response.status_code == 200, response.text
    json_data = response.json()
    assert "count" in json_data
    assert "next" in json_data
...
def test_get_list_of_event_areas():
    response = requests.get(f"{PARKKIOPAS_HOST}/public/v1/event_area/")
    assert response.status_code == 200, response.text
    json_data = response.json()
    assert json_data["type"] == "FeatureCollection"
    assert "count" in json_data
...
if __name__ == "__main__":
    ...
    if TEST_EVENT_PARKING:
        test_get_list_of_event_areas_statistics()
        test_get_list_of_event_areas()
        test_get_list_of_event_areas_total_statistics()
